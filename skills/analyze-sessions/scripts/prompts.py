#!/usr/bin/env python3
"""Dump user prompts across pi sessions for pattern analysis and reflection.
...
model that's looking for prompting patterns or user corrections.

Examples:
  python3 prompts.py --since 30d
  python3 prompts.py --since 7d --corrections
  python3 prompts.py --since 7d --max-chars 1500 --format jsonl
  python3 prompts.py --cwd /path/to/project --since 30d
...
CORRECTION_RE = re.compile(
    r"\b(never|always|wrong|stop|don't|do not|no,|instead|rule is|should be|must not|please don't|the rule)\b",
    re.IGNORECASE,
)
...
def main() -> int:
    ...
    p.add_argument("--corrections", action="store_true",
                   help="Filter prompts matching correction signals (never, always, wrong, stop, don't, etc.).")
    ...
    total_prompts = 0
    for s in summaries:
        prompts = _collect_prompts(s.path, args.max_chars, args.min_chars, args.corrections)
        if not prompts:
            continue
...
def _collect_prompts(path, max_chars: int, min_chars: int, corrections_only: bool = False):
    ...
    for rec in S.iter_records(path):
        ...
        if max_chars and n > max_chars:
            continue
        if corrections_only and not CORRECTION_RE.search(text):
            continue
