...
export default function (pi: ExtensionAPI) {
	...
	/**
	 * True when `text` is a slash invocation of a known extension command,
	 * `/skill:...`, or prompt template. Pi expands these only when the message
	 * starts with `/` and is exactly `/name [args]`.
	 */
	function isSlashInvocation(text: string): boolean {
		const match = text.match(/^\/([^\s]+)/);
		if (!match) return false;
		return pi.getCommands().some((command) => command.name === match[1]);
	}

	pi.on("input", async (event, ctx) => {
		if (enabled.size === 0) return; // continue unchanged

		// Slash invocations must stay message-initial: prepending a snippet hides
		// the leading "/" so pi never expands the command, and appending one gets
		// consumed as template arguments. Step aside and keep the toggles for the
		// next natural message instead of silently corrupting the invocation.
		if (isSlashInvocation(event.text)) {
			ctx.ui.notify("Prompt snippets skipped for slash command; toggles kept for your next message.", "info");
