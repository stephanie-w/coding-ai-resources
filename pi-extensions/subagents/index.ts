 ...
 *
 * Hybrid interface:
 *   - Unified meta-tool `subagent`:
 *       single:   { agent: "name", task: "..." }
 *       parallel: { tasks: [{ agent, task }, ...] }
 *       chain:    { chain: [{ agent, task: "... {previous} ..." }, ...] }
 *   - Direct dedicated tools: agents with `direct_tool: true` are also
 *     registered as their own tool (e.g. `py-explore` -> `py_explore`).
 *
 * Uses `--mode json` to stream structured output from subagents.
 */
...
import { Container, Key, matchesKey, Markdown, Spacer, Text } from "@earendil-works/pi-tui";
...
/** Compressed thinking levels for the `thinking` frontmatter integer 0-3. */
const THINKING_LEVELS = ["off", "low", "medium", "high"] as const;

function mapThinkingLevel(level: number): string {
	const n = Math.max(0, Math.min(3, Math.round(level)));
	return THINKING_LEVELS[n] ?? "off";
}

/** Turn an agent name into a valid tool identifier (e.g. `py-explore` -> `py_explore`). */
...
function formatToolCall(
	...
	switch (toolName) {
		...
		case "py_inspect": {
			...
			return themeFg("muted", "py_inspect ") + themeFg("accent", target);
		}
		default: {
...
interface SingleResult {
	agent: string;
	agentSource: "user" | "project" | "catalog" | "unknown";
	task: string;
	exitCode: number;
	messages: Message[];
	stderr: string;
...
/**
 * Resolve how to invoke the running pi binary across standalone binary, Bun,
 * and Node environments. `--append-system-prompt` accepts a file path (or
 * literal text); we write the system prompt to a temp file to avoid
 * argv-length limits.
 */
...
async function runSingleAgent(
	...
	if (agent.thinking !== undefined) {
		args.push("--thinking", mapThinkingLevel(agent.thinking));
	} else if (inheritsDispatchConfig && dispatchDefaults.thinkingLevel) {
		args.push("--thinking", dispatchDefaults.thinkingLevel);
	}
	if (agent.tools && agent.tools.length > 0) args.push("--tools", agent.tools.join(","));

	// Propagate subagent depth so bash-guard hard-blocks catastrophic commands.
	const currentDepth = Number(process.env.PI_SUBAGENT_DEPTH ?? "0");
	const childDepth = Number.isFinite(currentDepth) ? currentDepth + 1 : 1;
...
const DirectToolParams = Type.Object({
	task: Type.String({ description: "Task to delegate to the subagent" }),
	cwd: Type.Optional(Type.String({ description: "Working directory for the agent process" })),
});

/** Shared single-mode result builder used by both the unified tool and direct tools. */
...
function buildSubagentReport(discovery: ReturnType<typeof discoverAgents>, trusted: boolean): string {
	const lines: string[] = ["Subagents", ""];
	lines.push(
		discovery.projectAgentsDirs.length > 0
			? `project dirs: ${discovery.projectAgentsDirs.map(shortenPath).join(", ")}`
			: "project dirs: none",
	);
	lines.push("");
	...
	for (const a of discovery.agents) {
		const trustNote = a.source === "project" && !trusted ? " (not trusted)" : "";
		const exposures = ["subagent"];
		if (a.directTool) exposures.push(`direct: ${sanitizeToolName(a.name)}`);
		lines.push(`${a.name}  [${a.source}${trustNote}]  ${a.description}`);
		lines.push(
			`  model: ${a.model ?? "inherit"}${a.thinking !== undefined ? ` · thinking ${a.thinking} (${mapThinkingLevel(a.thinking)})` : ""}`,
		);
		lines.push(`  tools: ${a.tools && a.tools.length > 0 ? a.tools.join(", ") : "all default"}`);
		lines.push(`  exposed as: ${exposures.join(" + ")}`);
		lines.push("");
	}
	return lines.join("\n");
}
...
export default function (pi: ExtensionAPI) {
	...
	/** Register dedicated tools for agents with `direct_tool: true`. Idempotent. */
	function registerDirectTools(cwd: string, projectTrusted: boolean) {
		const discovery = discoverAgents(cwd, "both");
		const agents = projectTrusted
			? discovery.agents
			: discovery.agents.filter((a) => a.source !== "project");

		for (const agent of agents) {
			if (!agent.directTool) continue;
			const toolName = sanitizeToolName(agent.name);
			if (registeredDirectTools.has(toolName)) continue;

			pi.registerTool({
				name: toolName,
				label: `Subagent: ${agent.name}`,
				description: agent.description,
				promptSnippet: `Delegate a task to the "${agent.name}" subagent (isolated context window).`,
				promptGuidelines: agent.guidelines,
				parameters: DirectToolParams,
				async execute(_toolCallId, params, signal, onUpdate, ctx) {
					const trusted = ctx.isProjectTrusted();
					const runDiscovery = discoverAgents(ctx.cwd, "both");
					const runAgents = trusted
						? runDiscovery.agents
						: runDiscovery.agents.filter((a) => a.source !== "project");
					...
					const makeDetails = (results: SingleResult[]): SubagentDetails => ({
						mode: "single",
						agentScope: "both",
						projectAgentsDirs: runDiscovery.projectAgentsDirs,
						results,
					});
			...
			registeredDirectTools.add(toolName);
		}
	}

	// Register direct tools for user + catalog agents up front (project agents
	// need a known cwd and trust decision, so they register at session start).
	registerDirectTools(process.cwd(), false);

	pi.on("session_start", (_event, ctx) => {
		registerDirectTools(ctx.cwd, ctx.isProjectTrusted());
	});

	pi.registerCommand("subagents", {
		description: "List discovered subagents, sources, models, tools, and how they are exposed",
		handler: async (_args, ctx) => {
			const discovery = discoverAgents(ctx.cwd, "both");
			const report = buildSubagentReport(discovery, ctx.isProjectTrusted());
			...
			await ctx.ui.custom<void>(
				(tui, _t, _kb, done) => ({
					render(width: number) {
						...
						const frame = (l: string) => bar + l.slice(0, inner).padEnd(inner) + bar;
						const rows = lines.slice(scroll, scroll + maxView).map(frame);
	...
	pi.registerTool({
		...
		description: [
			...
			`Default agent scope is "user" (from ${path.join(getAgentDir(), "agents")} and ${path.join(getAgentDir(), "subagents")}).`,
			`To enable project-local agents in ${CONFIG_DIR_NAME}/agents or ${CONFIG_DIR_NAME}/subagents, set agentScope: "both" (or "project").`,
		].join(" "),
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (
				...
				if (projectAgentsRequested.length > 0) {
					...
					const dir = discovery.projectAgentsDirs[0] ?? "(unknown)";
